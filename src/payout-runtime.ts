...
export async function sendPayoutRuntime(env: Env, msg: PayoutMessage) {
  ...
  const entry = await env.DB.prepare(
    ...
    .first<{ amount: number }>();
  if (!entry) throw new Error(`celebration not confirmed yet: ${msg.dealId} ${msg.kind}`);
  ...
  if (mismatch) {
    ...
    ).bind(`rc_payout_${msg.dealId}`, msg.dealId).run();
    await env.DB.prepare(
      `UPDATE review_cases
  ...
  const inserted = await env.DB.prepare(
    `INSERT OR IGNORE INTO payouts (id, worker_id, amount, status, hold_reason)
     VALUES (?, ?, ?, ?, ?)`
  ).bind(payoutId, msg.workerId, entry.amount, held ? "held" : "queued", holdReason).run();

  const payout = await env.DB.prepare(
    `SELECT status, external_ref FROM payouts WHERE id=?`
  ).bind(payoutId).first<{ status: string; external_ref: string | null }>();
  ...
  ).bind(id, payoutId).run();
}

export async function consumePayoutBatch(batch: MessageBatch<PayoutMessage>, env: Env) {
