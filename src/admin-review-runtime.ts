...
export async function handleReviewResolveRuntime(
  request: Request,
  env: Env,
  reviewId: string
) {
  ...
  const held = body.verdict === "cleared"
    ? await env.DB.prepare(
        ...
        .bind(`po_${rc.deal_id}_%`)
        .all<{ id: string; worker_id: string; amount: number }>()
    : null;
  ...
  await env.DB.prepare(
    ...
    .bind(body.verdict, actor, note, reviewId)
    .run();
  ...
  if (body.verdict === "cleared") {
    ...
    const deal = await env.DB.prepare(`SELECT shop_id FROM deals WHERE id=?`)
      .bind(rc.deal_id)
